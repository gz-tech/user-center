...
/**
 * @Classname EmailValidCodeChecker
 * @Description
 * @Date 2022/3/31 6:01 下午
 * @Created by gz-d
 */
@Component
@Slf4j
public class NewDeviceUseEmailValidCodeChecker implements NewDeviceUseValideCodeChecker {
    @Autowired
    ...
    public void checkValidCode(LoginUseValidCodeRequest request) {
        ...
        String email = commonService.getEmailFromUserId(newDeviceLoginUtil.getUserIdFromSecret(request.getSecondLoginSecret()));
        validCodeUtils.check(ValidCodeCheckerTypeEnum.EMAIL, email, RequestContext.getDeviceId(),
                request.getValidCode(), ValidCodeType.NEW_DEVICE_EMAIL);
    }

    @Override
    public void checkTicket(String redisKey, String ticket) {
        ValueOperations<String, String> redisOpt = stringRedisTemplate.opsForValue();

        if ((ticket + "-" + LoginTypeEnum.PWD.getType()).equals(redisOpt.get(redisKey))) {
