 ...
 *
 */
public final class DefaultInjectionAttackHandler implements InjectionAttackHandler {
    ...
    private static final String SQL_INJECTION_1 = "(?:')|(?:--)|(/\\*(?:.|[\\n\\r])*?\\*/)|(?i)(\\b(and|exec|count|chr|"
            + "mid|master|or|truncate|char|declare|join|insert|select|delete|update|create|drop)\\b)";
    ...
    private static final String SPECIAL_CHARACTERS_1 = "(<)|(>)|(/\\*)|(\\*/)|(\\|)|(;)|(\\$)|(\\B\"\\B)|(')|(\\\\\")|"
            + "(\\(\\))|(\\B\\+\\B)"; // |(,)
    ...
    public String filterSqlInjection(String rawCharacters) {
        if (Strings.isNullOrEmpty(rawCharacters)) {
            return rawCharacters;
        }
        return RegExUtils.replaceAll(rawCharacters, SQL_INJECTION_REGEX, StringUtils.EMPTY);
    }
    ...
    public String filterXSSInjection(String rawCharacters) {
        ...
        return RegExUtils.replaceAll(rawCharacters, XSS_REGEX, StringUtils.EMPTY);
    }
    ...
    public String filterSpecialCharacters(String rawCharacters) {
        ...
        return RegExUtils.replaceAll(rawCharacters, SPECIAL_CHARACTERS_REGEX, StringUtils.EMPTY);
    }
    ...
    public void attackHandle(HttpServletRequest request, HttpServletResponse response, String parameters)
        ...
        try (PrintWriter out = response.getWriter()) {
            CommonResponse commonResponse = new CommonResponse(CommonErrorCode.PARAM_ERROR.getErrorCode(), "输入的参数包含非法字符");
            out.println(JsonUtility.toJson(commonResponse));
        }
    }
